...
import { computeAdherenceMarket } from "@/lib/scoring/adherence";
 ...
 *
 * Algoritmo: pool inteiro ponderado por freq bruta (TF-like). Captura long-tail.
 * Implementacao delegada a lib/scoring/adherence.js (computeAdherenceMarket)
 * desde a auditoria 2026-06-29 — antes era duplicado aqui e em 3 outros sites.
 *
 * Retorno mantido em shape legacy ({ valor, n_vagas, comuns }) pra nao quebrar
 * ScoreSnapshot.sub_scores e o teste tests/unit/subscores-deterministic.test.js.
 */
export function computeAderenciaVagas(profile, jobs) {
  const r = computeAdherenceMarket(profile?.skills, jobs);
  return { valor: r.adherence, n_vagas: r.n_vagas, comuns: r.comuns };
}
