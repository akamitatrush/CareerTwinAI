...
// NOISE_TOKENS movido pra `./role-utils` (compartilhado com fixtures.js —
// Fix P0.7 do report Data 2026-06-30 §Apendice C).
...
export async function searchJobs({ role, location = "Brasil", limit = 3 } = {}) {
  if (!role || typeof role !== "string") {
    // Role invalido: nao temos contexto pra buscar nada. Usamos `role: ""`
    // (early-return em fixtures.js:781 → catalogo inteiro slice limit) pra
    // amostra ilustrativa generica. Diferente de role nicho desconhecido
    // (esse cai no fluxo normal e retorna jobs=[] + noRelevantFixtures=true).
    return { jobs: await searchFixtures({ role: "", limit }), sources: ["fixtures"], noRelevantFixtures: false };
  }
...
async function _runSearch({ role, location, limit, cacheKey }) {
  ...
  //
  // 2026-06-30 (Gimli G3): se providers reais zeraram E fixtures tambem
  // (role fora das 20 categorias hardcoded), nao mais devolvemos "8 primeiros
  // do catalogo". Sinalizamos `noRelevantFixtures: true` pra UI mostrar
  // empty-state + form "pedir cobertura". Honestidade > engagement falso.
  let noRelevantFixtures = false;
  ...
  const payload = {
    ...
    illustrativeRatio,
    realCount,
    // Diferenciacao explicita:
    //   - illustrativeRatio === 1.0  → temos fixtures rotuladas (UI: aviso "ilustrativo")
    //   - noRelevantFixtures === true → role nicho sem cobertura (UI: empty + form)
    noRelevantFixtures,
  };
