...
export async function searchAdzuna({ role, location, limit = 3 }) {
  ...
  // Retry em 429/5xx — Adzuna rate-limita free tier em 1 RPS. Backoff exponencial
  // (~400ms, ~800ms) salva o radar quando o caller dispara em paralelo varios
  // searches. Erros 4xx (chave invalida, 403 Forbidden) NAO retornam — falham fast.
  let res;
  try {
    res = await withRetry(async () => {
      const r = await withTimeout(url);
      if (r.status === 429 || (r.status >= 500 && r.status <= 599)) {
        // Engatilha retry via throw — backoff entre tentativas.
        throw new Error(`adzuna: ${r.status}`);
      }
      return r;
    }, { maxAttempts: 2, baseDelayMs: 400 });
  } catch (e) {
    console.error(`adzuna: retry exauridos — ${e?.message || e}`);
    return [];
  }
  if (!res.ok) {
