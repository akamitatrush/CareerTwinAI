...
export async function exportUserData(userId) {
  ...
  const [
    ...
    // Audit log do user — LGPD principio da transparencia: user deve ver
    // quem/quando acessou seus dados, exports passados, mudancas de perfil.
    // actorIp omitido — eh hash, mas evita revelar pattern de acesso.
...
export async function eraseUserData(userId, { req, actorIp } = {}) {
  if (!userId) throw new Error("userId required");

  // Audit ANTES do delete — o FK ON DELETE SET NULL preserva o registro mesmo
  // se a row do user some, mas com userId=null. Registrar antes amarra a acao
  // ao user ainda existente (mais util pra forense). Cascade em todas as
  // relacoes apaga: Account, Session, Profile, ScoreSnapshot (-> Gap, PlanItem),
  // Consent, DataSource. Auth.js tambem ira invalidar a sessao.
  await audit({
    userId,
