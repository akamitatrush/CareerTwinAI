...
//
// Retry: 429/5xx no Voyage retentam ate 2x com backoff exponencial. Free tier
// limita 3 RPM — backoff de 400ms / 800ms ainda nao salva, mas evita falhar
// em rate limit transitorio. Caller (ingest) ja respeita batch-size + delay-ms.
//
...
async function embedVoyage(texts, inputType = "document") {
  ...
  if (!key) return null;
  // withRetry vai re-disparar este fetch em 429/5xx (mensagem do throw casa
  // com o pattern). 4xx (401, 422) falham fast — chave invalida nao melhora.
  const res = await withRetry(
    async () => {
      const r = await fetchWithTimeout(VOYAGE_URL, {
        method: "POST",
        headers: {
      ...
      if (r.status === 429 || (r.status >= 500 && r.status <= 599)) {
        ...
        throw new Error(`Voyage ${r.status}: ${errBody.slice(0, 200)}`);
      }
      return r;
    },
...
async function embedOpenAI(texts) {
  ...
  const res = await withRetry(
    async () => {
      ...
      if (r.status === 429 || (r.status >= 500 && r.status <= 599)) {
        const errBody = await r.text().catch(() => "");
        throw new Error(`OpenAI emb ${r.status}: ${errBody.slice(0, 200)}`);
      }
      return r;
    },
    { maxAttempts: 2, baseDelayMs: 600 }
...
export async function embedTexts(texts, { inputType = "document" } = {}) {
  ...
  if (process.env.VOYAGE_API_KEY) {
    try {
      ...
      voyageErr = e;
      console.error("voyage embed falhou:", e.message);
    }
  }
  if (process.env.OPENAI_API_KEY) {
    try {
      ...
      console.error("openai embed falhou:", e.message);
      throw new Error(
        `Embedding falhou em ambos providers. Voyage: ${voyageErr?.message || "(nao tentado)"}. OpenAI: ${e.message}`,
      );
    }
  }
  // Se chegou aqui: Voyage falhou e OpenAI nao configurado.
  if (voyageErr) {
    if (voyageErr.message.includes("429")) {
      throw new Error(
        "Voyage AI rate limit (3 RPM / 10K TPM no free tier sem cartao). " +
        "Solucoes: 1) Adicionar metodo de pagamento em voyageai.com (rate sobe pra 300 RPM); " +
        "2) Rodar com --batch-size 8 --delay-ms 22000 (respeita free tier); " +
        "3) Configurar OPENAI_API_KEY como fallback. " +
        `Detalhe: ${voyageErr.message}`,
      );
    }
    throw voyageErr;
  }
